...
show_status () {
    ...
    while read -r line; do
        menu+=([$line]="nordvpn status")
    done <<< $(nordvpn status)
    # dunstify "$(nordvpn status | sed 's/\r//g' | cut -d '-' -f 3 | sed 's/^ *//g')"
}
...
show_settings () {
    ...
    while read -r line; do
        if [ ! -z "${line// }" ]; then
            ...
            menu+=([$line]="nordvpn set $cmd $toggle && nordvpn settings")
        fi
...
show_menu () {
    ...
    if nordvpn status | grep -q 'Disconnected'; then
        ...
        menu+=([1 - Disconnect from NordVPN]="nordvpn disconnect" [3 - View Status]="nordvpn status")
    fi
    
    menu+=([2 - Settings]="nordvpn settings")
}
...
launcher=(rofi -no-lazy-grab -dmenu -theme themes/appsmenu.rasi Bi -lines 10 -p "${RTEXT}" -width 30 -location 0 -bw 1) 
